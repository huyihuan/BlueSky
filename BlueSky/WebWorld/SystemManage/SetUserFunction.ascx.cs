    ...
    public partial class SetUserFunction : System.Web.UI.UserControl
    {
        ...
        protected void rptItems_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                if (null != lbl)
                    lbl.Text = oItem.ParentId == -1 ? string.Format("<b>{0}</b>", oItem.Name) : string.Format("{0}{1}", " ".PadLeft((oItem.Level - 1) * 4, '　'), cbSelect.Checked ? string.Format("<a href='javascript:selectAction(\"{0}\",\"{1}\");'>{1}</a>", string.Format("{0}&userid={1}&setfn={2}", SystemUtil.ResovleSingleFormUrl(oItem.Id, "SetUserAction"), nUserId, oItem.Id), oItem.Name) : oItem.Name);
