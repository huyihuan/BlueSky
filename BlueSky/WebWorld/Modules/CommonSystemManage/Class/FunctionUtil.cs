...
namespace WebWorld.Modules.CommonSystemManage.Class
{
    public class FunctionUtil
    {
        ...
        private static FunctionItem[] Functions = null;
        public static string CreateFunctionTree()
        {
            int nCurrentUserId = SystemUtil.GetCurrentUserId();
            if (nCurrentUserId <= 0)
            {
                //开发初期避免频繁的登录操作，默认拥有所有的权限
                Functions = FunctionItem.GetFunctions(-1, true);
            }
            else
            {
                UserRoleItem[] alUserRoles = UserRoleItem.GetUserRoles(nCurrentUserId);
                List<string> ltUserRoleIds = new List<string>();
                if (null != alUserRoles)
                {
                    foreach (UserRoleItem item in alUserRoles)
                        ltUserRoleIds.Add(item.RoleItemId + "");
                }
                Functions = FunctionItem.GetFunctions(-1, ltUserRoleIds.ToArray(), true);
            }
            htParentIdToCount = FunctionItem.GetParentIdToCount(Functions);

            foreach (FunctionItem funcItem in Functions)
        ...
        {
            return string.Format("<span class=\"{0}\" onclick=\"nodeClick(this);\"{1}>{2}</span>", __ClassName, __NodeTag, __Content);
        }

        private static string __CreateCommonSpan(string __ClassName, string __NodeTag, bool __bClick)
        {
            string strClick = __bClick ? " onclick=\"nodeClick(this);\"" : "";
