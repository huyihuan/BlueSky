        ...
        public override string ToString()
        {
            ...
            switch (SqlType)
            {
                case HSqlType.Select:
                    sb.Append(string.Format("select {0} from {1} ", this.SqlContent, this.SqlTableName));
                    break;
                case HSqlType.Update:
                    sb.Append(string.Format("update {0} set {1} ", this.SqlTableName, this.SqlContent));
                    break;
                case HSqlType.Delete:
                    sb.Append(string.Format("delete from {0} ", this.SqlTableName));
                    break;
                case HSqlType.Insert:
                    sb.Append(string.Format("SET IDENTITY_INSERT [{0}] ON;", SqlTableName));
                    sb.Append(string.Format("insert into {0}({1}) values({2});", SqlTableName, SqlContent, SqlWhereString));
                    break;
            }
            ...
            if (SqlType != HSqlType.Insert && !string.IsNullOrEmpty(SqlWhereString))
            {
                if (!(SqlType == HSqlType.Select && this.SqlContent.IndexOf("ROW_NUMBER") != -1))
                    sb.Append(" where ");
                sb.Append(string.Format(" {0}", this.SqlWhereString));
            }
            if(SqlType == HSqlType.Insert)
                sb.Append(string.Format("SET IDENTITY_INSERT [{0}] OFF;", SqlTableName));

            _strSqlString = sb.ToString();
