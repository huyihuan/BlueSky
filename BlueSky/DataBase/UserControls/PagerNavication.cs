...
namespace DataBase.UserControls
{
    ...
    public class PagerNavication : WebControl, INamingContainer
    {
        ...
        private void InitChildControls()
        {
            ...
            string[] astrSizes = strPageSizeList.Split(new char[] { ',', '#', '$', '_', '&', '*' });
            foreach (string strSize in astrSizes)
            {
                this._ddlSizeList.Items.Add(strSize);
            }
            this._ddlSizeList.SelectedValue = (astrSizes.Contains(this.PageSize + "") ? this.PageSize : _DefaultSize) + "";
